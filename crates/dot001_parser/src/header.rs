...
impl BlendFileHeader {
    ...
    /// Read header from a byte slice for zero-copy parsing
    pub fn read_from_slice(data: &[u8]) -> Result<Self> {
        ...
        let byte_7 = data[7];
        let is_legacy_header = byte_7 == b'_' || byte_7 == b'-';

        if is_legacy_header {
            let file_format_version = 0;
            let pointer_size = if byte_7 == b'_' { 4 } else { 8 };

            let byte_8 = data[8];
            let is_little_endian = match byte_8 {
                b'v' => true,
                b'V' => false,
                _ => {
                    return Err(Error::blend_file(
                        format!("Invalid endian indicator: {}", byte_8 as char),
                        crate::error::BlendFileErrorKind::InvalidHeader,
                    ));
                }
            };

            let version_bytes = &data[9..12];
            let version = std::str::from_utf8(version_bytes)
                .map_err(|_| {
                    Error::blend_file(
                        "Invalid version format",
            ...
            if data.len() < 17 {
                return Err(Error::blend_file(
                    "Data too small for v1 header",
                    crate::error::BlendFileErrorKind::InvalidHeader,
                ));
            }

            let byte_8 = data[8];
            let header_size = (byte_7 - b'0') * 10 + (byte_8 - b'0');
            if header_size != 17 {
                return Err(Error::blend_file(
                    format!("Unknown header size: {header_size}"),
                    crate::error::BlendFileErrorKind::InvalidHeader,
            ...
            let endian_indicator = data[12];
            if endian_indicator != b'v' {
                return Err(Error::blend_file(
                    "Expected 'v' endian indicator",
                    crate::error::BlendFileErrorKind::InvalidHeader,
                ));
