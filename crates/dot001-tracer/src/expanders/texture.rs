/// Modernized Texture expander using hybrid_expander macro
...
hybrid_expander! {
    TextureExpander, b"TE\0\0", "Tex" => {
        ...
        custom: |block_index, blend_file, dependencies| {
            // Read the texture block data for custom type-specific logic
            if let Ok(texture_data) = blend_file.read_block_data(block_index) {
                if let Ok(reader) = blend_file.create_field_reader(&texture_data) {
