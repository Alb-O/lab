...
import { loggerDebug, loggerInfo, loggerWarn, loggerError, initLogger, registerLoggerClass } from '@utils';
...
export default class Fragments extends Plugin implements IFragmentsPlugin {
	...
	async onload() {
		...
		loggerInfo(this, 'Plugin onload started');

		// Load settings
		this.settings = Object.assign({}, DEFAULT_SETTINGS, await this.loadData());
		loggerDebug(this, 'Settings loaded:', this.settings);
		...
		loggerDebug(this, 'Components initialized');
		...
		this.app.workspace.onLayoutReady(() => {
			loggerDebug(this, 'Layout ready, starting initial video detection');
			setTimeout(() => {
		...
		loggerInfo(this, 'Plugin onload completed');
	}
	private registerEvents() {
		loggerDebug(this, 'Registering plugin events');
		// Register for file changes to update video detection
		this.registerEvent(
			this.app.workspace.on('active-leaf-change', (leaf) => {
				loggerDebug(this, 'Active leaf changed');
		...
		this.registerEvent(
			this.app.metadataCache.on('changed', (file) => {
				loggerDebug(this, 'Metadata changed for file:', file?.path);
		...
		this.registerEvent(this.app.workspace.on('layout-change', () => {
			loggerDebug(this, 'Layout changed');
			this.handleLayoutChange();
		}));
	}

	private setupUIComponents() {
		loggerDebug(this, 'Setting up UI components');
	...
	public onunload() {
		loggerInfo(this, 'Plugin unloading');
		...
		loggerInfo(this, 'Plugin unload completed');
	}
	 ...
	 */	public detectVideosInAllDocuments(): VideoWithFragment[] {
		loggerDebug(this, 'Starting video detection in all documents');
		const allDocuments = this.getAllRelevantDocuments();
		...
		loggerDebug(this, `Found ${markdownViews.length} markdown views to scan`);
		...
		loggerDebug(this, `Detected ${allVideos.length} videos total`);
	...
	async saveSettings() {
		loggerDebug(this, 'Saving settings');
