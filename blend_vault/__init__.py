...
def register():
    ...
    package_name = __package__ or "blend_vault"  # Fallback for development to match manifest
    ...
    for module_path in submodules_to_reload:
        try:
            importlib.reload(importlib.import_module(module_path, package=__package__))
        except ImportError:
    ...
    for module_path in MODULES_TO_REGISTER:
        try:
            module = importlib.reload(importlib.import_module(module_path, package=__package__))
...
def unregister():
    ...
    for event, entries in HANDLERS.items():
        handler_list = getattr(bpy.app.handlers, event)
        for module_path, fn_name in entries:
            fn = globals().get(fn_name) or getattr(importlib.import_module(module_path, package=__package__), fn_name)
