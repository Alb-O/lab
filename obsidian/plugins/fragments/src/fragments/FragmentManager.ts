...
import { parseFragmentToSeconds, loggerDebug } from '@utils';
...
export class FragmentManager {
	...
	public applyFragmentRestrictions(videosFromMarkdown: VideoWithFragment[], targetDocuments: Document[]): void {
		loggerDebug(this, `Applying fragment restrictions to ${videosFromMarkdown.length} videos across ${targetDocuments.length} documents`);
		...
		for (const doc of targetDocuments) {
			...
			loggerDebug(this, `Found ${allVideoElementsInDom.length} video elements in DOM`);
		...
		loggerDebug(this, `Fragment restriction application completed`);
	}
		...
		if (parentEmbed) {
			...
			if (parentSrcAttr) {
				const currentSrcPathPart = parentSrcAttr.split('#t=')[0];                const srcTimeMatch = parentSrcAttr.match(/#t=([^,]+)(?:,([^,]+))?/);                if (srcTimeMatch && srcTimeMatch[1] && srcTimeMatch[1] !== '0.001') {
					...
					loggerDebug(this, `Extracting fragment from embed: start="${srcTimeMatch[1]}", end="${srcTimeMatch[2] || ''}"`);
					const parsedStart = parseFragmentToSeconds(srcTimeMatch[1]);
					loggerDebug(this, `  Parsed start: ${JSON.stringify(parsedStart)}`);
					...
					if (srcTimeMatch[2] && srcTimeMatch[2] !== '0.001') {
						...
						loggerDebug(this, `  Parsed end: ${JSON.stringify(parsedEnd)}`);
